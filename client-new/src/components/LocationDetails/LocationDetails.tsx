...
import { LocationDetails as LocationDetailsType } from "../../interfaces/LocationDetails";
...
const buildRecommendString = (locationDetails: LocationDetailsType) => {
  const reviewsLength =
    typeof locationDetails.reviews !== undefined && locationDetails.reviews
      ? locationDetails.reviews.length
      : 0;

  let recommendString = `${Utility.decimalToPercent(
    Utility.calculateScore(locationDetails.upvotes, locationDetails.downvotes)
...
const scoreColor = (locationDetails: LocationDetailsType) => {
  if (!locationDetails.reviews || locationDetails.reviews.length === 0) {
    return grey[500];
  }

  let color;
  const score = Utility.calculateScore(
...
const renderReviews = (locationReviews: Array<LocationReview>) => (
  <Box style={{ width: "100%" }}>
...
const renderPage = (locationDetails: LocationDetailsType) => (
  <Fragment>
      ...
      <Paper variant="outlined">
        ...
        <List>
          ...
          <ListItem>
            <ThumbsUpDownIcon
              style={{
                color: scoreColor(locationDetails),
                paddingRight: "12px",
              }}
            />
    ...
    {locationDetails.reviews &&
      ...
      renderReviews(locationDetails.reviews)}
  </Fragment>
);
...
function LocationDetails() {
  ...
  if (placeId && !locationDetails && !error) {
    ...
    placeId &&
    locationDetails &&
    recentLocationReviews.length > 0 &&
    !error
  ) {
    // Data received with details and reviews
    content = renderPage({
      ...locationDetails,
      reviews: recentLocationReviews,
    });
  } else if (
    placeId &&
    locationDetailsFromMaps &&
    recentLocationReviews.length === 0
  ) {
